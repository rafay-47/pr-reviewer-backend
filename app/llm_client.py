...
def _build_findings_markdown(
    ...
    """Build the markdown comment for the PR with collapsible sections."""
    needs_review = needs_review or []
    fingerprints = fingerprints or []
    resolved_findings = resolved_findings or []
    
    # Collect fingerprints for deduplication
    ...
    if filtered_by_policy and total_before_filter > len(findings):
        lines.append(f"> ℹ️ *{total_before_filter - len(findings)} finding(s) were filtered by repository policy.*")
        lines.append("")
    ...
    if resolved_findings_count > 0 or resolved_findings:
        ...
        lines.append("")
        lines.append(f"**{resolved_findings_count or len(resolved_findings)} issue(s) from the previous review have been fixed!**")
        lines.append("")
        
        if resolved_findings:
            for rf in resolved_findings:
                risk_emoji = {"HIGH": "🔴", "MEDIUM": "🟠", "LOW": "🟡"}.get(rf.get("risk", ""), "⚪")
                title = rf.get("title", "Unknown issue")
                file_path = rf.get("file_path", rf.get("file", "unknown"))
                line_range = rf.get("line_range", "")
                lines.append(f"- {risk_emoji} ~~{title}~~ (`{file_path}:{line_range}`)")
            lines.append("")
    ...
    if not findings and not needs_review:
        lines.append("✅ No security vulnerabilities identified in this change.")
        lines.append("")
        lines.append("---")
        lines.append("*This review was performed by AI AppSec PR Reviewer*")
        return "\n".join(lines)
    
    # Group findings by status (new vs still present) and risk level
    new_findings = [f for f in findings if f.is_new]
    still_present = [f for f in findings if not f.is_new]
    ...
    stats_parts = []
    if high_findings:
        stats_parts.append(f"🔴 {len(high_findings)} High")
    if medium_findings:
        stats_parts.append(f"🟠 {len(medium_findings)} Medium")
    if low_findings:
        stats_parts.append(f"🟡 {len(low_findings)} Low")
    if needs_review:
        stats_parts.append(f"🔵 {len(needs_review)} Needs Review")
    
    if stats_parts:
        lines.append(f"**Findings:** {' | '.join(stats_parts)}")
        
    # Show new vs still present stats
    if still_present:
        lines.append(f"**Status:** 🆕 {len(new_findings)} New | 🔄 {len(still_present)} Still Present")
    
    lines.append("")
    
    # List confirmed findings
    for i, finding in enumerate(findings, 1):
        risk_emoji = {"HIGH": "🔴", "MEDIUM": "🟠", "LOW": "🟡"}.get(finding.risk.value, "⚪")
        confidence_emoji = {"HIGH": "🟢", "MEDIUM": "🟡", "LOW": "🟠"}.get(finding.confidence.value, "⚪")
        status_badge = "🆕 NEW" if finding.is_new else "🔄 Still Present"
        
        lines.append(f"### {i}. {risk_emoji} [{finding.risk.value}] {finding.title}")
        lines.append("")
        lines.append(f"**📁 File:** `{finding.file_path}:{finding.line_range}` | **Status:** {status_badge}")
        lines.append(f"**🎯 Confidence:** {confidence_emoji} {finding.confidence.value}")
        lines.append("")
        
        lines.append(f"**❌ Issue:** {finding.description}")
        lines.append("")
        lines.append(f"**⚠️ Impact:** {finding.impact}")
        lines.append("")
        lines.append(f"**✅ Recommendation:** {finding.recommendation}")
        lines.append("")
        
        # Evidence in collapsible block
        if finding.evidence:
            lines.append("<details>")
            lines.append("<summary><b>📋 Evidence</b></summary>")
            lines.append("")
        ...
        # Example fix in collapsible block
        if finding.example_fix:
            lines.append("<details>")
            lines.append("<summary><b>💡 Example Fix</b></summary>")
            lines.append("")
            lines.append("```javascript")
    ...
    if needs_review:
        ...
        lines.append("")
        lines.append("> *The following items were flagged but need human verification. They may be false positives.*")
        lines.append("")
        
        for i, finding in enumerate(needs_review, 1):
            lines.append("<details>")
            lines.append(f"<summary><b>{i}. {finding.title}</b> — `{finding.file_path}:{finding.line_range}`</summary>")
            lines.append("")
            if finding.evidence:
                lines.append("```javascript")
                lines.append(finding.evidence)
    ...
    lines.append("*This review was performed by AI AppSec PR Reviewer. Please validate findings before taking action.*")
...
def _build_error_markdown(error_type: str, details: str) -> str:
    """Build a clean markdown error message for LLM failures."""
    error_messages = {
        "timeout": "⏱️ The AI review request timed out.",
        "api_error": "🔌 The AI service returned an error.",
        "server_error": "🖥️ The AI service is temporarily unavailable.",
        "connection_error": "🌐 Could not connect to the AI service.",
        "max_retries_exceeded": "🔄 The AI review failed after multiple retry attempts.",
        "internal_error": "⚠️ An unexpected error occurred.",
    }
    
    message = error_messages.get(error_type, "⚠️ The AI review could not be completed.")
