...
def _build_findings_markdown(
    ...
    """Build a production-grade markdown comment for a PR security review."""
    ...
    lines = [
        ...
        f"- High: **{len(high_findings)}** | Medium: **{len(medium_findings)}** | Low: **{len(low_findings)}**",
        f"- Needs manual review: **{len(needs_review)}**",
        f"- New: **{len(new_findings)}** | Still present: **{len(still_present)}**",
        "",
    ]

    if filtered_by_policy and total_before_filter > len(findings):
        lines.append(f"> Note: {total_before_filter - len(findings)} finding(s) were filtered by repository policy.")
        lines.append("")

    if resolved_findings_count > 0 or resolved_findings:
        lines.append("### Resolved Since Previous Review")
        lines.append("")
        lines.append(f"- Resolved findings: **{resolved_findings_count or len(resolved_findings)}**")
    ...
    if not findings and not needs_review:
        lines.append("### Result")
        lines.append("")
        lines.append("No security vulnerabilities were identified in this change set.")
        lines.append("")
        lines.append("---")
        lines.append("_Generated by AppSec PR Reviewer_")
        return "\n".join(lines)
    ...
    for i, finding in enumerate(findings, 1):
        ...
        lines.append(f"#### {i}. [{finding.risk.value}] {finding.title}")
        lines.append("")
    ...
    if needs_review:
        lines.append("### Items Requiring Manual Verification")
        lines.append("")
        lines.append("The following items require human validation before remediation decisions are made.")
        lines.append("")
    ...
    lines.append("---")
    lines.append("_Generated by AppSec PR Reviewer. Validate findings before applying changes._")
    return "\n".join(lines)
